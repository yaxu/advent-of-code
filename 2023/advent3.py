...
for (y, line) in enumerate(lines):
    for match in re.finditer(r'\d+', line):
        ...
        number = match[0]
        border = ""
        minx = max(match.start()-1, 0)
        maxx = min(match.end()+1, width-1)
        if y > 0:
            border += lines[y-1][minx:maxx]
        if x > 1:
            border += lines[y][match.start()-1]
        if match.end() < width:
            border += lines[y][match.end()]
        if y < (width-1):
            border += lines[y+1][minx:maxx]
        print(match, border)
        if re.search(r'[^0-9\.]', border):
            print("aho")
            print(number)
            total += int(number)
        else:
